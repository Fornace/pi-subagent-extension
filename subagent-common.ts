...
export function hasReportedUsage(usage: UsageStats): boolean {
	return usage.input > 0 || usage.output > 0 || usage.cacheRead > 0 || usage.cacheWrite > 0;
}

export function formatManagedUsage(usage: UsageStats, compact = false): string {
...
export function formatManagedCost(usage: UsageStats): string {
...
export function formatManagedTokens(usage: UsageStats): string {
	if (usage.turns > 0 && !hasReportedUsage(usage)) return "Unavailable (provider did not report usage)";
